...
function login_user()
{
   ...
   {
     return $msg_to_user = "Pogrešan username ili password.";
   }
...
function make_reservation( $destination_id = 0 )
{
   ...
   if ( $destination )
   {
      $reservations_left = get_reservatons_left( $destination );
      if ($reservations_left > 0)
      {
         ...
         else
         {
           ...
           {
             $msg_to_user = "Uspešno ste rezervisali destinaciju: " . $destination->name;
           }
   ...
   {
      $msg_to_user = "Destinacija ne postoji.";
   }
